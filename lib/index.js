...
function createStore(customReducer, defaultStore, shouldComposeWithDevTools, middlewares) {
  ...
  function rootReducer(state, action) {
    if (state === undefined) {
      if (defaultStore) state = defaultStore;else return {};
    }

    if ('type' in action && action.type === 'SET_PROP') {
      return setterReducer(state, action);
    } else if ('type' in action && action.type === 'SET_PROP_ASYNC') ; else if (customReducer) {
...
function connect(mapStateToProps, mapDispatchToProps) {
  const rootMapDispatchToProps = (dispatch, ownProps) => {
    ...
    if (typeof mapDispatchToProps === 'function') {
      mdtp = Object.assign({}, { ...mapDispatchToProps(dispatch, ownProps)
      });
